...
def load_pdf(pdf_file):
    ...
    for doc in docs:
        doc.page_content = remove_non_utf8_chars(doc.page_content)
    return docs
...
class BaseLoader:
    def __init__(self) -> None:
        self.num.processes = get_num_cpu()
...
class Loader:
    ...
    def load(self, pdf_files: Union[str, List[str]], wokers: int = 1):
        if isinstance(pdf_files, str):
            pdf_files = [pdf_files]
        doc_loaded = self.doc_loader(pdf_files, wokers)
